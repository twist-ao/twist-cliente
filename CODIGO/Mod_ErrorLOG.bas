...
'                End If
        ...
        Case "U5" ' <<--- CHOTS | Golpe critico
            rData = Right$(rData, Len(rData) - 2)
            Call AddtoRichTextBox(frmMain.RecTxt, "Es un Golpe Crítico! (+" & rData & ")", 65, 190, 156, True, False, False)
            Exit Sub
    ...
    Select Case Left$(sData, 3)
        ...
        Case "UON" 'CHOTS | Users Online, Records, miembros del clan
            rData = Right$(rData, Len(rData) - 3)
            Call AddtoRichTextBox(frmMain.RecTxt, "Número de usuarios: " & Val(ReadField(1, rData, 44)) & ". Record de Usuarios Conectados Simultaneamente: " & Val(ReadField(2, rData, 44)), 65, 190, 156, False, False)
