...
Sub PeneVenoso(ByVal semen As String)
    ...
    Select Case sData
        ...
        Call AddtoRichTextBox(frmMain.RecTxt, "6: Destrucción", 0, 128, 255, True, False, False)
        Exit Sub
        Case "Z96"
...
'                End If
        ...
        Case "U5" ' <<--- CHOTS | Golpe critico
            semen = Right$(semen, Len(semen) - 2)
            Call AddtoRichTextBox(frmMain.RecTxt, "Es un Golpe Crítico! (+" & semen & ")", 65, 190, 156, True, False, False)
            Exit Sub
        ...
        Case ServerPackages.crearChar
            ...
            End If
            charlist(charindex).ClanId = Val(ReadField(13, semen, 44))
            charlist(charindex).Criminal = Val(ReadField(14, semen, 44))
            charlist(charindex).priv = Val(ReadField(15, semen, 44))
    ...
    Select Case Left$(sData, 3)
        ...
        Case "CXF"                  ' >>>>> Mostrar FX sobre Personaje :: "CFX"
            ...
            possibleWav = Val(ReadField(4, semen, 44))
        ...
        Case "TAU" 'CHOTS | Crea Torneo AUTO
            ...
            torneoCupo = Val(ReadField(1, semen, 44))
            torneoTipo = ReadField(2, semen, 44)
            Call AddtoRichTextBox(frmMain.RecTxt, "[TORNEO AUTOMÁTICO]", 0, 128, 255, True, False, False)
            Call AddtoRichTextBox(frmMain.RecTxt, "Tipo: " & torneoTipo, 0, 128, 255, True, False, False)
            Call AddtoRichTextBox(frmMain.RecTxt, "Nivel mínimo: 46", 0, 128, 255, True, False, False)
            Call AddtoRichTextBox(frmMain.RecTxt, "Nivel máximo: 54", 0, 128, 255, True, False, False)
            Call AddtoRichTextBox(frmMain.RecTxt, "Cupo máximo: " & torneoCupo, 0, 128, 255, True, False, False)
        ...
        Case "UON" 'CHOTS | Users Online, Records, miembros del clan
            semen = Right$(semen, Len(semen) - 3)
            Call AddtoRichTextBox(frmMain.RecTxt, "Número de usuarios: " & Val(ReadField(1, semen, 44)) & ". Record de Usuarios Conectados Simultaneamente: " & Val(ReadField(2, semen, 44)), 65, 190, 156, False, False)
        ...
        Case "CNC" 'CHOTS | Conecta
            ...
            SkillPoints = Val(ReadField(25, semen, 64))
        ...
        Case "MUE" '"Muere o Renace"
            ...
            If Val(ReadField(1, semen, 44)) = 0 Then
        ...
        Case "CHO" 'CHOTS | Has Ganado x monedas de oro
            semen = Right$(semen, Len(semen) - 3)
            Call AddtoRichTextBox(frmMain.RecTxt, "Has ganado " & semen & " Monedas de Oro!", 255, 255, 0, True, False)
            Exit Sub
...
Case "VES" 'CHOTS | Optimización de clicks a usuarios
            ...
            Genero = Val(ReadField(11, semen, 44))
            Clase = Val(ReadField(12, semen, 44))
            Raza = Val(ReadField(13, semen, 44))
...
#End If
    ...
    Select Case Left$(sData, 7)
        ...
        Case "ABESPIA" 'CHOTS | Sistema de Espías
            ...
            frmEspia.hp.Width = (Val(ReadField(2, semen, 44)) / Val(ReadField(3, semen, 44))) * 1320
            frmEspia.lblHp.Caption = Val(ReadField(2, semen, 44)) & "/" & Val(ReadField(3, semen, 44))
            frmEspia.man.Width = (Val(ReadField(4, semen, 44)) / Val(ReadField(5, semen, 44))) * 1320
    ...
    Select Case UCase$(Left$(semen, 8))
        Case "EXPILIST" 'CHOTS | Sistema de Espia
            semen = Right$(semen, Len(semen) - 8)
            If Not ESPIA_PAUSADO Then Call frmEspia.lstEspia.AddItem(semen)
        Exit Sub
...
Sub VaginaJugosa(ByVal flujoVaginal As String)
    ...
    If AuxCmd = "DEMSG" And Len(flujoVaginal) > 1000 Then
        Exit Sub
